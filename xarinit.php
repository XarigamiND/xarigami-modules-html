...
function html_init()
{
    ...
    *****************************************************************/
    ...
    //This initialization takes us to version 1.4.0 - continue in upgrade
    return html_upgrade('1.4.0');
}
...
function html_upgrade($oldversion)
{
    ...
    switch ($oldversion) {
        ...
        case '1.4':
        case '1.4.0':
        //register the new reference transform hook
            if (!xarModRegisterHook(
                'item', 'transform', 'API',
                'html', 'user', 'transformrefs')
                ) {return;}
                //set the var
                xarModSetVar('html','transformrefs', true);
        case '1.4.2':
            if (!xarModRegisterHook('module', 'modifyconfig', 'GUI',
                                    'html', 'admin', 'modifyconfighook')) {
                return false;
            }
            if (!xarModRegisterHook('module', 'updateconfig', 'API',
                                    'html', 'admin', 'updateconfighook')) {
                return false;
            }
            xarModSetVar('html', 'allowhookoverrides', false);
        ...
        case '1.4.3':
         ...
         // Set up database tables
            ...
            list($htmltypeid, $htmltypename) = $result->fields;
            if (!isset($htmltypeid)) {
                xarLogMessage('HTML MODULE: Upgrade - no htmltypeid');
                return;
            }

           //update elements for HTML5
           // The default values of the HTML tags are:
            //   0 = Not allowed
            //   1 = Allowed
            //   2 = Allowed with attributes
            $htmltags = array('article'     =>  2,
                              'aside'       =>  2,
                              'command'     =>  0,
                              'details'     =>  0,
                              'summary'     =>  0,
...
function html_delete()
{
    ...
    if (!xarModUnregisterHook('item','transform-input','API',
                              'html','user','transforminput')) return;

    if (!xarModUnRegisterHook('item','transform','API',
                            'html','user', 'transformoutput')) return;
